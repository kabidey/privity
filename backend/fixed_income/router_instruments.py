...
def _is_valid_isin_format(query: str) -> bool:
    """Check if query looks like an ISIN code"""
    import re
    query = query.upper().strip()
    # Indian ISIN: INExxxxxxxxx or INxxxxxxxxxx (G-Sec)
    return bool(re.match(r'^IN[A-Z0-9]{10,12}$', query))
    ...
    **Live Lookup Feature:**
    ...
    if not results and live_lookup and _is_valid_isin_format(query):
        ...
        try:
            ...
            live_lookup_result = {
                "success": False,
                "message": f"Live lookup error: {str(e)}"
            }
    ...
    # Mark instruments that are already imported (skip live lookup results as they're already marked)
    for result in results:
        if not result.get("live_lookup"):
    ...
    # Add live lookup info if attempted
    if live_lookup_attempted:
        response["live_lookup_attempted"] = True
        response["live_lookup_result"] = live_lookup_result
