...
from utils.demo_isolation import add_demo_filter, mark_as_demo
...
async def create_purchase(
    ...
    purchase_doc = {
        ...
        "price_per_share": round(purchase_data.price_per_unit, 2),
        "total_amount": round(purchase_data.quantity * purchase_data.price_per_unit, 2),
        "status": "pending",
...
async def delete_purchase(
    purchase_id: str,
    current_user: dict = Depends(get_current_user),
    _: None = Depends(require_permission("purchases.delete", "delete purchases"))
):
    """Delete a purchase order (requires purchases.delete permission)"""
    user_role = current_user.get("role", 6)
