from fastapi import FastAPI, APIRouter, HTTPException, Depends, status, UploadFile, File, Form, Request, WebSocket, WebSocketDisconnect, Query, Response, Body
...
# ============== Client Document & Portfolio Endpoints (Not in router) ==============
...
# NOTE: Client CRUD endpoints (GET /clients, GET /clients/{id}, PUT /clients/{id}, 
# DELETE /clients/{id}, suspend/unsuspend) have been moved to routers/clients.py
...
# ============== Client Booking Confirmation Endpoints ==============
# (Public endpoint - no auth required, client confirms via email link)
...
# NOTE: PUT /bookings/{id}/approve has been moved to routers/bookings.py
...
async def approve_loss_booking(
    ...
    if approve and booking.get("approval_status") == "approved":
        if client and client.get("email"):
            frontend_url = os.environ.get('FRONTEND_URL', 'https://booking-manager-52.preview.emergentagent.com')
            await send_templated_email(
...
# NOTE: PUT /bookings/{id}/void has been moved to routers/bookings.py
