        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <Card className="border-l-4 border-l-indigo-500">
            <CardContent className="pt-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">Total BP Payments</p>
                  <p className="text-lg md:text-xl font-bold text-indigo-600">
                    {formatCurrency(bpPayments.reduce((sum, p) => sum + (p.payment_amount || 0), 0))}
                  </p>
                </div>
                <Users className="h-6 w-6 text-indigo-500 opacity-50" />
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {bpPayments.length} payments
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-purple-500">
            <CardContent className="pt-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">Pending BP Payments</p>
                  <p className="text-lg md:text-xl font-bold text-purple-600">
                    {formatCurrency(bpPayments.filter(p => p.status === 'pending' || p.status === 'processing').reduce((sum, p) => sum + (p.payment_amount || 0), 0))}
                  </p>
                </div>
                <Clock className="h-6 w-6 text-purple-500 opacity-50" />
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {bpPayments.filter(p => p.status === 'pending' || p.status === 'processing').length} pending
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-teal-500">
            <CardContent className="pt-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">Paid BP Payments</p>
                  <p className="text-lg md:text-xl font-bold text-teal-600">
                    {formatCurrency(bpPayments.filter(p => p.status === 'paid').reduce((sum, p) => sum + (p.payment_amount || 0), 0))}
                  </p>
                </div>
                <CheckCircle className="h-6 w-6 text-teal-500 opacity-50" />
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {bpPayments.filter(p => p.status === 'paid').length} paid
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <TabsContent value="bp-payments">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track revenue share payments for Business Partners
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0 md:p-6">
              {bpPayments.length === 0 ? (
                <p className="text-center py-8 text-muted-foreground">No BP payments found.</p>
              ) : (
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {bpPayments.map((payment) => (
                        <TableRow key={payment.id} data-testid={`bp-payment-row-${payment.id}`}>
                          <TableCell className="font-mono text-sm">{payment.booking_number}</TableCell>
                          <TableCell>
                            <div>
                              <p className="font-medium">{payment.bp_name}</p>
                              {payment.bp_pan && <p className="text-xs text-muted-foreground">{payment.bp_pan}</p>}
                            </div>
                          </TableCell>
                          <TableCell>{payment.client_name}</TableCell>
                          <TableCell className="font-mono">{payment.stock_symbol}</TableCell>
                          <TableCell className="text-right">{formatCurrency(payment.profit)}</TableCell>
                          <TableCell className="text-right text-purple-600">{payment.bp_share_percent}%</TableCell>
                          <TableCell className="text-right font-bold text-purple-600">
                            {formatCurrency(payment.payment_amount)}
                          </TableCell>
                          <TableCell>{getBpPaymentStatusBadge(payment.status)}</TableCell>
                          <TableCell>
                            {payment.payment_reference ? (
                          ...
                          <TableCell>
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => openBpPaymentDialog(payment)}
                              data-testid={`update-bp-payment-${payment.id}`}
                            >
                              <Edit className="h-4 w-4" />
                            </Button>
                          </TableCell>
